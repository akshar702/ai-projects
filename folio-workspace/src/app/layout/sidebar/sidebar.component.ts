import {
  Component, ChangeDetectionStrategy, inject, signal, OnInit, HostListener
} from '@angular/core';
import { CommonModule } from '@angular/common';
import { Router } from '@angular/router';
import { Store } from '@ngrx/store';
import { toggleSidebar, setActiveSection, setSidebarCollapsed } from '../../core/store/app.actions';
...
@Component({
  ...
  styles: [`
    ...
    .label.hidden-label { opacity: 0; max-width: 0; pointer-events: none; }
    .label.visible-label { opacity: 1; max-width: 160px; }

    /* On mobile: sidebar floats as fixed overlay so main content gets full width */
    .sidebar-fixed {
    ...
    /* Scrim shown behind open sidebar on mobile */
    .mobile-scrim {
      position: fixed;
      inset: 0;
  ...
  `],
  template: `
    <!-- Scrim — only on mobile when sidebar is expanded -->
    @if (isMobile() && !collapsed()) {
      <div class="mobile-scrim" (click)="onToggle()"></div>
    }
    ...
    <nav
      ...
      style="background: var(--bg-surface); border-right: 1px solid var(--border); overflow: hidden; position: relative; z-index: 10;">

      <!-- Logo -->
      <div class="flex items-center px-3 py-4 border-b flex-shrink-0"
           style="border-color: var(--border); min-height: 60px">
        <div class="w-8 h-8 rounded-lg flex-shrink-0 flex items-center justify-center text-white font-bold text-sm"
             style="background: linear-gradient(135deg, var(--primary), var(--accent))">F</div>

        <span class="label ml-3 font-semibold text-base"
              [class.hidden-label]="collapsed()"
              [class.visible-label]="!collapsed()"
              style="color: var(--text-primary)">Folio</span>
      ...
      <div class="px-4 pt-4 pb-2 flex-shrink-0">
        <span class="label text-xs font-semibold uppercase tracking-widest"
              [class.hidden-label]="collapsed()"
              [class.visible-label]="!collapsed()"
              style="color: var(--text-dim)">Workspace</span>
      </div>
      ...
      <div class="flex-1 px-2 space-y-0.5 overflow-y-auto">
        <div *ngFor="let item of navItems"
          ...
          <span class="label text-sm"
                [class.hidden-label]="collapsed()"
                [class.visible-label]="!collapsed()"
                style="color: inherit">{{ item.label }}</span>
        </div>
      </div>
      ...
      <div class="px-2 py-3 flex-shrink-0 border-t" style="border-color: var(--border)">
        <div (click)="navigate(settingsItem)"
          ...
          <span class="label text-sm"
                [class.hidden-label]="collapsed()"
                [class.visible-label]="!collapsed()"
                style="color: inherit">Settings</span>
        </div>
      </div>
...
export class SidebarComponent implements OnInit {
  ...
  settingsItem: NavItem = { icon: '⚙️', label: 'Settings', route: '/settings', section: 'settings' };

  ngOnInit(): void {
    // Auto-collapse on mobile at startup
    if (this.isMobile()) {
      this.store.dispatch(setSidebarCollapsed({ collapsed: true }));
    }
  }
